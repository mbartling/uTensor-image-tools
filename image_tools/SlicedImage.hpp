...
template<typename T>
SlicedImage<T>::SlicedImage(const uTensor::Tensor& t, unsigned int num_row_slices,
    unsigned int num_col_slices)
...
//const T operator()(uint16_t y, uint16_t x, uint16_t c) const {
//  return static_cast<T>(image(0, y + num_row_slices*sliced_height, x + num_col_slices*sliced_width, c));
//}
// Readonly
template<typename T>
T SlicedImage<T>::operator()(uint16_t y, uint16_t x, uint16_t c){
  return static_cast<T>(image(0, y + cur_slice_r*sliced_height, x + cur_slice_c*sliced_width, c));
}
